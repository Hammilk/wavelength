...
export function scoreGuess(target, guess) {
  ...
  if (distance <= 4) return { distance, points: 4 };
  if (distance <= 10) return { distance, points: 3 };
  if (distance <= 16) return { distance, points: 2 };
  if (distance <= 22) return { distance, points: 1 };
